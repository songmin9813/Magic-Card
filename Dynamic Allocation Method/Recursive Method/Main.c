...
*/
void recur(int CARD, int QUAN, int **arr, int i, int z, int power)
{
	...
	recur(CARD, QUAN, arr, i, z + 1, power);//재귀 함수 호출
}
int main()
{
	...
	printf("시작하기 전에 몇 장의 카드를 가지고 시작할 것인지 알려주세요>>");
	scanf("%d", &CARD);//사용자에게 카드의 개수를 입력받습니다.
	//startTime = clock();
	QUAN = (1 << CARD);
	card = (int **)malloc(sizeof(int *)*CARD);//배열의 동적 할당
	for (i = 0; i < CARD; i++)
		card[i] = (int *)malloc(sizeof(int)*QUAN);
	for (i = 0; i < CARD; i++) {//행에 먼저 루프를 걸어줍니다.
		z = 0;
		power = 1 << i;
		recur(CARD, QUAN, card, i, z, power);//재귀 함수를 호출합니다.
	}
